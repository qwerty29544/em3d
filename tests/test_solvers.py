...
def test_solver_action_budgets_stop_all_methods(backend_numpy_double):
    problem = _toy_problem_for_solver(backend_numpy_double)
    ...
    configurations = {
        "SIM": SolverConfig(
            max_iter=100,
            max_operator_actions=2,
            mu=2.0 + 0.0j,
            radius=1.0,
        ),
        "BiCGStab": SolverConfig(max_iter=100, max_operator_actions=2),
        "TwoStep": SolverConfig(max_iter=100, max_operator_actions=2),
    }
    solvers = {
        "SIM": SIM(configurations["SIM"]),
        "BiCGStab": BiCGStab(configurations["BiCGStab"]),
        "TwoStep": TwoStep(configurations["TwoStep"]),
    }
    for name, solver in solvers.items():
        result = solver.solve(operator, rhs)
        assert result.operator_action_count <= 2, name
        assert result.status in {"converged", "max_operator_actions"}
